...
* modify : 2018-10-20
*=================================================================
* This file was created from the php documentation of amazon and several tutorials
* It contains the s3 class and all the functions necessary for the proper functioning of the challenge coveo
*=================================================================
*/
...
class S3
{
	// Déclarations de constantes ACL
	...
	* @return array | false
	*/
	public static function listBuckets($detailed = true)
	{
	...
	*/
	...
	public static function getBucketSize($bucket, $prefix = null, $marker = null, $maxKeys = null, $delimiter = null, $returnCommonPrefixes = false)
	{
		$rest = new S3Request('GET', $bucket, '', self::$endpoint);
		if ($maxKeys == 0) $maxKeys = null;
		if ($prefix !== null && $prefix !== '') $rest->setParameter('prefix', $prefix);
	...
	*/
	...
	public static function getObjectInformation($bucket, $returnInfo = true)
	{
		$rest = new S3Request('HEAD', $bucket, self::$endpoint);
		$rest = $rest->getResponse();
		if ($rest->error === false && ($rest->code !== 200 && $rest->code !== 404))
			$rest->error = array('code' => $rest->code, 'message' => 'Unexpected HTTP status');
		if ($rest->error !== false)
		{
			self::__triggerError(sprintf("S3::getObjectInfo({$bucket}): [%s] %s",
			$rest->error['code'], $rest->error['message']), __FILE__, __LINE__);
			return false;
		}
		return $rest->code == 200 ? $returnInfo ? $rest->headers : true : false;
